...
import styles from './RatingFunctionality.module.css'
...
const RatingFunctionality = ({ movieId, readOnly = false, initialRating = 0 }: RatingFunctionalityProps) => {
  ...
  useEffect(() => {
    if (!readOnly && movieId) {
      ...
      setLoading(false)
      setCurrentRating(initialRating ? initialRating / 2 : 0)
    }
  }, [movieId, readOnly, initialRating])
  ...
  function getStarFill(starIndex: number): 'empty' | 'half' | 'full' {
    ...
    if (readOnly) {
      // In read-only mode, only show full and half stars (no empty)
      if (rating >= starValue) return 'full'
      if (rating >= starValue - 0.5) return 'half'
      // Don't render empty stars in read-only mode
      return 'empty'
    }
    ...
    return (
      <div className={styles.container}>
        <p className={styles.loginPrompt}>
          <Link href="/authentication">Sign in</Link> to rate and add to watchlist
        </p>
      </div>
    )
  }
  ...
  return (
    <div className={readOnly ? styles.readOnlyContainer : styles.container}>
      {/* Watchlist Button */}
      {!readOnly && (
      <button
      ...
      <div className={styles.ratingContainer}>
        ...
        >
          ...
          {[0, 1, 2, 3, 4].map((starIndex) => {
            ...
            // In read-only mode, skip rendering empty stars
            if (readOnly && fillType === 'empty') return null
            ...
            return (
              <div
                ...
                onMouseMove={!readOnly ? (e) => handleStarHover(starIndex, e) : undefined}
                onClick={!readOnly ? (e) => {
