...
const UserWatchlistPageContent = () => {
  ...
  const handleSortSelect = (value: string) => {
    ...
    const newUrl = `/watchlist/${username}?sortBy=${value}`
    router.push(newUrl)
    
  }

  async function fetchWatchlist(page: number = 1, sortOrder: string = 'title_asc') {
    if (!username) {
      setLoading(false)
      return
    }
    ...
    try {
      ...
      const { data: countData, error: countError } = await supabase
        .rpc('get_user_watchlist_count', { p_user_id: userData.id })
